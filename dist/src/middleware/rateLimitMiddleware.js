...
const redis = REDIS_URL ? new ioredis_1.default(REDIS_URL, { lazyConnect: true, maxRetriesPerRequest: 1 }) : null;
let redisReady = false;
function keyFrom(req, suffix) {
    const ip = req.ip || req.socket.remoteAddress || "unknown";
    return `${ip}:${suffix}`;
}
async function checkRedisLimit(key, maxRequests, windowMs) {
    if (!redis)
        return null;
    try {
        if (!redisReady) {
