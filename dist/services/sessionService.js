...
async function issueSessionCookies(res, req, userId) {
    ...
    const accessToken = Buffer.from(JSON.stringify({ sub: userId, exp: Math.floor(accessExpiry.getTime() / 1000) })).toString("base64url");
...
async function rotateSessionFromRefresh(req, res) {
    ...
    const accessExpiry = nextAccessExpiryUtc();
    const newAccess = Buffer.from(JSON.stringify({ sub: existing.userId, exp: Math.floor(accessExpiry.getTime() / 1000) })).toString("base64url");
