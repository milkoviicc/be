...
exports.getOrCreateTodayBudget = getOrCreateTodayBudget;
exports.refreshTodayBudgetSpent = refreshTodayBudgetSpent;
const prismaClient_1 = require("../utils/prismaClient");
const client_1 = require("@prisma/client/runtime/client");
const date_1 = require("../utils/date");
async function getOrCreateTodayBudget(userId) {
    // Store UTC, but keep the app's calendar day stable (e.g. Europe/Zagreb).
    const dateOnly = (0, date_1.utcMidnightForAppDate)();
    let budget = await prismaClient_1.prisma.dailyBudget.findUnique({
    ...
    const [user, goals] = await Promise.all([
        ...
        prismaClient_1.prisma.savingsGoal.findMany({
            where: { userId },
            orderBy: { createdAt: "asc" },
        }),
    ]);
    let baseDailyAllowance = new client_1.Decimal(0);
    if (user?.currentBalance) {
        ...
        if (baseDailyAllowance.lessThan(0))
            baseDailyAllowance = new client_1.Decimal(0);
    }
        ...
        const otherFixed = user.monthlyOtherFixedCosts ?? new client_1.Decimal(0);
        const fixedCosts = housing.add(utilities).add(otherFixed);
        let variableIncome = income.sub(fixedCosts);
        if (variableIncome.lessThan(0))
            variableIncome = new client_1.Decimal(0);
        let totalGoalSavingsPerMonth = new client_1.Decimal(0);
        for (const goal of goals) {
            const targetStart = new Date(Date.UTC(goal.targetDate.getUTCFullYear(), goal.targetDate.getUTCMonth(), goal.targetDate.getUTCDate())).getTime();
            const remainingAmount = goal.goalAmount.sub(goal.startingBalance ?? new client_1.Decimal(0));
        ...
        if (leftoverForSpendingPerMonth.lessThan(0))
            leftoverForSpendingPerMonth = new client_1.Decimal(0);
    ...
    const yesterday = await prismaClient_1.prisma.dailyBudget.findUnique({
        where: { userId_date: { userId, date: yesterdayDate } },
    });
    const carryOverFromPrev = yesterday ? yesterday.computedLimit.sub(yesterday.spentToday) : new client_1.Decimal(0);
    ...
    if (yesterday && user) {
        ...
        const isConsecutiveDay = user.lastStreakDate &&
            new Date(Date.UTC(user.lastStreakDate.getUTCFullYear(), user.lastStreakDate.getUTCMonth(), user.lastStreakDate.getUTCDate())).getTime() === yesterdayDate.getTime();
    ...
    const existingSpent = budget?.spentToday ?? new client_1.Decimal(0);
    const finalComputedLimit = baseDailyAllowance.add(carryOverFromPrev).sub(existingSpent);
    budget = await prismaClient_1.prisma.dailyBudget.upsert({
        where: { userId_date: { userId, date: dateOnly } },
        create: {
            userId,
            date: dateOnly,
            baseDailyAllowance,
            carryOverFromPrev,
            spentToday: new client_1.Decimal(0),
            computedLimit,
...
async function refreshTodayBudgetSpent(userId) {
    const dateOnly = (0, date_1.utcMidnightForAppDate)();
    ...
    const agg = await prismaClient_1.prisma.transaction.aggregate({
        where: {
            userId,
            isExcluded: false,
            amount: { lt: 0 },
    ...
    const spentToday = (agg._sum.amount ?? new client_1.Decimal(0)).abs();
    const computedLimit = budget.baseDailyAllowance.add(budget.carryOverFromPrev).sub(spentToday);
    return prismaClient_1.prisma.dailyBudget.update({
        where: { userId_date: { userId, date: dateOnly } },
